...
function load_abc(abc_string) {
    ...
    tunebook = ABCJS.renderAbc(notation_display.id, abc_string, {
        ...
        scale: DEFAULT_SCALE,
        add_classes: true,
        paddingtop: 10,       // Extra padding for spacing
        paddingbottom: 10,
    });
...
function load_playlist_file(filename) {
    ...
    $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
            ...
            clear_playlist();
            playlist_files = data.split('\n').filter(line => line.trim() !== '');
            playlist_index = 0;
            for (var i = 0; i < playlist_files.length; i += 1) {
                playlist_files[i] = "./music/"+playlist_files[i]
                playlist.append('<li class="list-group-item" data-playlist-index="' + i + '">' + playlist_files[i] + '</li>');
            }
